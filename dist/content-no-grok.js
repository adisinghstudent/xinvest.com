// X Invest Extension - Content Script (No Grok Dependency Version)
...
(function () {
    ...
    function createInvestTab() {
        // Try to find Grok tab first, otherwise use any nav link as template
        const grokSelector = 'nav[aria-label="Primary"] a[href="/i/grok"]';
        const fallbackSelector = 'nav[aria-label="Primary"] a[href*="/"]';

        // Try Grok first
        let templateTab = document.querySelector(grokSelector);
        let insertAfter = templateTab;

        // If no Grok, use any nav link
        if (!templateTab) {
            console.log('X Invest: Grok tab not found, using fallback template');
            const allNavLinks = document.querySelectorAll(fallbackSelector);
            // Use the last nav link as template (usually Lists or Premium)
            templateTab = allNavLinks[allNavLinks.length - 1];
            insertAfter = templateTab;
        }

        if (!templateTab) {
            console.log('X Invest: Could not find any navigation template');
            return;
        }

        waitForElement(fallbackSelector, () => {
            ...
            const investTab = templateTab.cloneNode(true);
          ...
          `;
            ...
            // Insert after the template tab (or at the end of nav)
            if (insertAfter && insertAfter.parentElement) {
                insertAfter.parentElement.insertBefore(investTab, insertAfter.nextSibling);
            }
